...
Error FileSystem::mount()
{
	...
	create(*mountString, S_IRUSR | S_IWUSR);
...
Error FileSystem::processRequest(FileSystemRequest *req)
{
	...
	if ((cache = findFileCache(&path)) || cache = lookupFile(&path))
	{
		file = cache->file;
	}
...
void FileSystem::clearFileCache(FileCache *cache)
{
	...
	{
		//Traverse subtree if it isn't ivalidated yet.
		if (i.current().valid)
		{
	...
	//Remove the entry itself, if emtpy.
