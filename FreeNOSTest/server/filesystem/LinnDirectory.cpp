...
LinnDirectory::LinnDirectory(LinnFileSystem *f, LinnInode *i)
{
	m_size = inode->size;
	m_access = inode->mode;
}
...
Error LinnDirectory::read(IOBuffer & buffer, Size size, Size offset)
{
	...
	for (u32 ent = 0; ent < inode->size / sizeof(LinnDirectoryEntry); ent++)
	{
		...
		if ((blk=(ent * sizeof(LinnDirectoryEntry))/sb->blockSize) >= LINN_INODE_DIR_BLOCKS)
		{
			break;
		}
		...
		//Get the next entry.
		if (fs->getStorage()->read(off, &dent, sizeof(LinnDirectoryEntry))<0))
		{
			return EACCES;
		}
...
bool LinnDirectory::getLinnDirectoryEntry(LinnDirectoryEntry *dent, const char *name)
{
	LinnSuperBlock *sb = fs->getSuperBlock();
	u64 offset;
	//Loop all blocks.
	for (u32 blk = 0; blk < LINN_INODE_NUM_BLOCKS(sb, inode); blk++)
	{
		...
		{
			//Calculate offset to read.
			offset = (inode->block[blk] * sb->blockSize) + (sizeof(LinnDirectoryEntry)*ent);
			//Get the next entry.
